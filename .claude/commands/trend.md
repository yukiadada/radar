...
```bash
python3 fetch/market.py --report --date <기준일> --days 90
```

출력(기준 설명 + 표)을 그대로 쓴다. 표의 판정(방향대로/반대로/보합/아직)을 축별로 세어 한 줄로 적는다. `가격 자료 없음` 이면 그 한 줄만.
...
- 소스 편향을 밝힌다: 정책 축은 문서형 소스(federal_register, fed_press, wh_actions, sec·ftc·doj·ustr·doe, court_opinions)가 많아 구조적 시그널이 많이 나온다. 사회 축은 공식 통계·정기 조사(bea_releases, census_indicators, pew_research)만 구조적이라 건수가 적다. 축 간 건수 차이를 그대로 힘의 차이로 읽지 않는다.
- 시장 반응은 사건 전날 종가 대비 티커 평균의 SPY 대비 초과수익이다. "방향대로"가 많은 축·섹터는 시장이 그 근거를 같은 쪽으로 읽었다는 뜻이고, "반대로"는 근거와 가격이 어긋난다는 뜻이다(근거가 틀렸거나, 이미 반영됐거나, 다른 힘이 컸거나). 어느 쪽인지 단정하지 않고 확인 대기 항목으로 넘긴다. 표본이 5건 미만이면 비율을 말하지 않는다.
