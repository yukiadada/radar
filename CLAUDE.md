...
> /save 자동 업데이트 — 2026-09-18 14:45

**브랜치:** main
**마지막 커밋:** c92cbdb 정렬 점수를 유효기간 기준 누적으로: 시그널은 분기 90일·1년 365일·다년 730일 동안 살아 있고 가중치가 직선으로 0까지 줄어듦 (scoring.py PARAMS validity). 사이트·브리프·/trend 의 기본 화면이 누적(수준), 최근 30일 창은 새 근거(변화). 90·180·365일 창은 장부가 (n−7)일 쌓이면 자동으로 열림 (enabled_windows). 섹터 상세에 남은 비중 표시. axes.md 계산식 절, CLAUDE.md·brief.md·trend.md·review.md 를 같은 기준으로

**미완료 항목:**
- 정렬 점수는 2026-09-18 부터 유효기간 기준 누적(scoring.py PARAMS validity 분기 90·1년 365·다년 730일, 가중치 직선 감쇠). 사이트·브리프·/trend 기본 화면이 누적(수준), 최근 30일 창은 새 근거(변화). 90·180·365일 창은 장부가 (n−7)일 쌓이면 자동으로 열림 — 180일 2026-12-14, 1년 2027-06-17 께. 계산식 설명은 axes.md "3축 정렬"
- 9/19 루틴 브리프가 "## 섹터 정렬 (누적)" 제목과 ledger.py 의 누적 표로 나오는지 확인. 시장 반응 90일 열은 12월쯤 표본이 차면 추가. 몇 달 뒤 누적 %가 너무 안 움직이거나 튀면 validity·감쇠 모양 재검토
- 사이트(site/index.html): 이름 Radar. 글꼴 IBM Plex Sans KR 단일(Google Fonts, 막히면 시스템 고딕), 화면마다 h1 구조, 설명은 "기호 읽는 법" 펼침, 모서리 3단계(상자 12·조작 8·배지 6), 섹터·섹터 상세·추이의 기준 탭(누적·창)은 머리글 아래 8px 고정(배경 없음). 새 화면은 pageHead·secHead·legendBox 부품과 토큰을 쓴다. 긴 대시(—)·알약 모양·색 띠·반투명 흐림은 쓰지 않는다
- 오늘 탭: 제목·설명 → 오늘의 브리프(축별 카드만, 카드가 200px 넘으면 그라데이션 + 글자 버튼 "더 보기") → 방향별 정렬(누적) → 쉬운 말로 읽는 지금의 방향 → 섹터 한눈에(누적) → 지난 브리프. 요약 4칸·날짜 줄·정보 칩은 뺐다. 섹터 정렬·버린 뉴스·30일 누적·시장 반응·맵 수정 제안은 브리프 탭에만. 브리프 "쉬운 말로"의 이득·손해는 `이득 (티커): 이유` / `손해 (티커): 이유` 줄
...
- 사이트 상단에 빌드 경고 배너가 보이면 그 자료(장부 줄·sector_map·브리프 형식·가격 자료 5일 이상 오래됨·이득/손해 줄의 티커)를 고친다. 빌드는 멈추지 않는다
