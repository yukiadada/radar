...
- 정렬 % 계산: 가중치 w = impact × horizon(분기 1, 1년 2, 다년 3). 누적(기본)은 유효기간(분기 90일, 1년 365일, 다년 730일) 안의 시그널만 세고 w 에 남은 비중(1 − 지난 일수 ÷ 유효기간)을 곱한다. 창(최근 30일 등)은 창 안의 시그널을 그대로 센다. 축 점수 = clamp(Σ 방향×w ÷ 6, −1, +1) 을 소수 2자리로. 정렬 % = 50 + 50 × 세 축 평균 (axes.md "3축 정렬"). 상태는 3축 정렬 / 2축 정렬 / 1축 / 엇갈림 / 양쪽 / 역풍 / 신호 없음. `fetch/scoring.py` 가 유일한 구현이다.
...
> /save 자동 업데이트 — 2026-09-18 15:47

**브랜치:** main
**마지막 커밋:** 380e0e4 코드리뷰 10건 반영: 축 점수를 소수 2자리로 반올림해 점수·상태·표시 일치(감쇠로 0.005 미만인 축이 상태에서만 세던 문제), 확인 대기는 살아 있는 시그널 기준(90일 창이 오래된 + 시그널을 놓치던 문제), fetch.yml 종가 갱신 실패를 PIPESTATUS 로 잡음, marked 스크립트 SRI, build.py 가 키 없는 장부 줄에 KeyError 대신 경고, 일별 차트 날짜 라벨 간격을 날짜 수에 비례, 기준 탭에 유효기간 칩, backfill paged_feed ua 전달, ledger append 검증을 assert 대신 명시적 검사, CLAUDE.md·review.md 계산 설명에 유효기간·누적
...
- 2026-09-18 코드리뷰 10건 반영(380e0e4): 축 점수 2자리 반올림, 확인 대기는 살아 있는 시그널 기준, fetch.yml 종가 갱신 실패 경고(PIPESTATUS), marked SRI(버전 올릴 때 integrity 해시도 같이), 일별 차트 라벨 간격, build.py 키 없는 줄 경고. 9/19 fetch.yml 종가 갱신 스텝이 경고 없이 끝나는지 확인
