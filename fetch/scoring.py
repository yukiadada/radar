...
정렬 점수 (framework/axes.md "3축 정렬"):
  ...
  정렬 % = 50 + 50 × (축 점수 합 ÷ 축 수). 창 안에 시그널이 없으면 None.
  ...
  추이(series) = 날마다 그날을 기준일로 같은 창을 다시 계산한 값. 장부 첫날 전은 만들지 않는다.
...
PARAMS = {"K": 6.0, "HW": {"분기": 1, "1년": 2, "다년": 3}, "windows": [30, 90], "span_days": 90, "delta_days": 7}
DIRV = {"+": 1, "-": -1, "±": 0}
HW = PARAMS["HW"]
K = PARAMS["K"]
...
def _tally(w: list[dict], smap: dict) -> tuple[dict, collections.Counter]:
    """창 안 행을 섹터×축으로 모은다. sector_map 에 없는 섹터·축의 행은 orphans 에 센다 (맵에서 이름이 바뀌거나 빠진 섹터)."""
...
def series(rows: list[dict], today: datetime.date, days: int, span_days: int = PARAMS["span_days"], smap: dict | None = None) -> dict:
    """날짜별 정렬 %. dates 는 오래된 순, 장부 첫날(또는 span_days 전) 부터 today 까지. 신호 없는 날은 None. 장부가 비면 dates 도 비어 있다."""
    smap = load_sector_map() if smap is None else smap
    dirs = directions(smap)
